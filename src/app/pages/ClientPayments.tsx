...
export default function ClientPayments() {
  ...
  async function submitReview(row: WorkOrder) {
    ...
    setBusyId("");
    if (rpcError) { setError(rpcError.message); return; }
    setNotice("Review submitted for moderation. It will be labeled as a verified YardPilotUSA project if published.");
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-7">
      <div><h1 className="flex items-center gap-3 text-2xl font-bold text-slate-900 dark:text-white"><CreditCard size={24} /> Accepted projects & payments</h1><p className="mt-1 text-sm text-slate-500">See the selected company, keep communication in YardPilotUSA, review the estimate and invoice, and preserve cancellation history.</p></div>
