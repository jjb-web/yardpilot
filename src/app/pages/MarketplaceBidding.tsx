...
export default function MarketplaceBidding() {
  const { authUserId, activeWorkspace, activeWorkspaceId, role } = useApp();
  ...
  async function loadWorkspaceMarketplaceState() {
    ...
    const [bidsResult, profileResult, workResult] = await Promise.all([
      supabase.from("client_job_bids").select("*").eq("workspace_id", activeWorkspaceId),
      supabase
        .from("marketplace_business_profiles")
        .select("published, accepting_client_work, city, state")
        .eq("workspace_id", activeWorkspaceId)
        .maybeSingle(),
      supabase.rpc("get_workspace_marketplace_work_orders", { requested_workspace_id: activeWorkspaceId }),
    ]);

    const firstError = bidsResult.error || profileResult.error || workResult.error;
    if (firstError) setError(firstError.message);

    setOwnBids((bidsResult.data ?? []) as ClientJobBid[]);
    setAcceptedWork((workResult.data ?? []) as MarketplaceWorkOrder[]);

    const profile = profileResult.data as BusinessProfileState | null;
  ...
  async function search(
    reset = true,
    overrides?: { city?: string; state?: string },
  ) {
  ...
  useEffect(() => {
    ...
    async function initialize() {
      setError("");
      const profile = await loadWorkspaceMarketplaceState();
      if (cancelled) return;
      const initialCity = profile?.city?.trim() || "";
      const initialState = profile?.state?.trim() || "";
      setCity(initialCity);
      setState(initialState);
      await search(true, { city: initialCity, state: initialState });
    }
  ...
  function openBid(request: ClientJobRequest) {
    const existing = bidByRequest[request.id];
    setSelectedRequest(request);
    setBidAmount(existing?.amount != null ? String(existing.amount) : "");
    setBidMessage(existing?.message ?? "");
    setProposedStart(existing?.proposed_start ?? "");
  }
  ...
  async function submitBid() {
    ...
    setError("");
    setMessage("");
    const { error: upsertError } = await supabase.from("client_job_bids").upsert(
      {
  ...
  function clearLocation() {
    setCity("");
    setState("");
    void search(true, { city: "", state: "" });
  }

  return (
    <div className="space-y-6">
      <div>
        <h2 className="flex items-center gap-2 text-xl font-bold text-slate-900 dark:text-white"><DollarSign size={21} /> Client bidding market</h2>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Clients publish projects and local companies compete by submitting offers.</p>
      </div>
      ...
      {!canBid && (
        <div className="rounded-xl border border-slate-200 bg-white p-3 text-sm text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
          Employees may browse requests, but only owners, co-owners, and managers can submit a company bid.
        </div>
      )}
      ...
      {acceptedWork.length > 0 && (
        <section className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5 shadow-sm dark:border-emerald-900 dark:bg-emerald-950/20">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white">Accepted marketplace projects</h3>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Create an estimate from the accepted request, then continue through the existing job and invoice workflow.</p>
          <div className="mt-4 space-y-3">
            {acceptedWork.map((work) => (
              <article key={work.work_order_id} className="rounded-xl border border-emerald-200 bg-white p-4 dark:border-emerald-900 dark:bg-slate-900">
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">{work.work_status.replaceAll("_", " ")}</p>
                    <h4 className="mt-1 font-bold text-slate-900 dark:text-white">{work.request_title}</h4>
                    <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{[work.city, work.state, work.postal_code].filter(Boolean).join(", ") || "Location not listed"}</p>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
          <div>
            <h3 className="font-bold text-slate-900 dark:text-white">Search client projects</h3>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">The initial location comes from the active company listing. Only 20 matches load at a time.</p>
          </div>
      ...
      <div className="space-y-4">
        {requests.map((request) => {
          ...
          return (
            <article key={request.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">{request.service_type || "Landscaping project"}</p>
                  <h3 className="mt-1 text-lg font-bold text-slate-900 dark:text-white">{request.title}</h3>
                  <p className="mt-1 flex items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400"><MapPin size={15} /> {[request.city, request.state, request.postal_code].filter(Boolean).join(", ") || "Location not listed"}</p>
                  <p className="mt-3 whitespace-pre-line text-sm leading-6 text-slate-600 dark:text-slate-300">{request.description}</p>
                ...
                <button type="button" onClick={() => openBid(request)} disabled={!canBid || !businessWorkspace || !profileReady} className="rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:bg-slate-200 disabled:text-slate-500 dark:bg-emerald-700 dark:disabled:bg-slate-700">
                  {existingBid ? "Update bid" : "Submit bid"}
                </button>
              </div>
              {existingBid && <p className="mt-4 rounded-lg bg-slate-50 p-3 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300"><strong>Your bid:</strong> {existingBid.status}{existingBid.amount != null ? ` · $${Number(existingBid.amount).toLocaleString()}` : ""}</p>}
            </article>
          );
        })}
        {!loading && requests.length === 0 && <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-400">No client projects matched this search. Clear the city and state filters to verify the request is open.</div>}
      </div>

      {hasMore && <button type="button" onClick={() => void search(false)} disabled={loading} className="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200">Load 20 more</button>}

      {selectedRequest && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-900">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Bid on {selectedRequest.title}</h3>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Offer amount
              ...
              <label className="text-sm font-medium text-slate-700 dark:text-slate-200">Proposed start
              ...
              <label className="sm:col-span-2 text-sm font-medium text-slate-700 dark:text-slate-200">Proposal
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button type="button" onClick={() => setSelectedRequest(null)} className="rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-semibold text-slate-700 dark:border-slate-600 dark:text-slate-200">Cancel</button>
              <button type="button" onClick={() => void submitBid()} disabled={busy} className="inline-flex items-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50 dark:bg-emerald-700">{busy ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />} Submit bid</button>
            </div>
          </div>
        </div>
      )}
